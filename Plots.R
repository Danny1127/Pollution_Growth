...
#library(ggrepel)
tp <- gsub("[ /()]","_",names(GDP_PM25_CSC))
names(GDP_PM25_CSC) <- gsub("__scale.*100_","",tp)
long_diff <- NULL

 
### calculate long difference 
for (FinalYear in 2010:2017){
  ...
  long_diff <- rbind(long_diff,long_diff0)
}

### save data
save2all(mydata=long_diff,filename="long_diff",filetype="all")
# 
...
#   for (FinalYear in 2010:2017){
#     InitialYear <- FinalYear - 10
#     long_diff0 <- subset(long_diff,Year=FinalYear)
#     
